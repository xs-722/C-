...
void lookfor(SS dd[], int N)
{
	long int a;
	scanf("%d", &a);
	for(int i=0;i<N;i++)
	{
		long int b = (long int)dd[i].number;
		if(a==b)
		{
			printf("第%d名信息 学号：%s	姓名:%s		总成绩:%4.2f分\n", i + 1, &(dd[i].number[0]), &(dd[i].name[0]), dd[i].generalScore);
		}
	}
}
void bb(SS dd[], int N)
{
	...
	for(int i=0;i<N;i++)
	{ 
		c = c + (dd[i].generalScore - b)*(dd[i].generalScore - b);
	}
	int d;
	d = c / N;
	printf("%d", d);
